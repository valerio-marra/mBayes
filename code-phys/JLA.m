...
(* data vector *)
datajlaBin=Import["cats/JLA/jla_mub.txt","Table","HeaderLines"->1];
ztabBin=datajlaBin[[All,1]];
{zminBin,zmaxBin}=MinMax[ztabBin];(*Row[{"z range:",{zminBin,zmaxBin}}];*)
mutabBin=datajlaBin[[All,2]];

(* covariance matrix *)
CovImp=Flatten[Import["cats/JLA/jla_mub_covmatrix.txt","Table"]];
NjlaBin=CovImp[[1]];
Dimensions[covmuBin=Partition[CovImp[[2;;]],NjlaBin]];
(*covmuBin=DiagonalMatrix[Diagonal[covmuBin]];*) (* for tests - keeps only diagonal terms *)
